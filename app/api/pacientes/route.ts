...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabase.from("pacientes").select("*").order("created_at", { ascending: false }).limit(100)
...
export async function POST(req: Request) {
  ...
  const rl = rateLimit(`pacientes:POST:${ip}`, 20, 60_000)
  ...
  const { data, error } = await supabase.from("pacientes").insert(toInsert).select("*").single()
