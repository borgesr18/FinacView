...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabase.from("matriculas").select("*").order("data_inicio", { ascending: false }).limit(100)
...
export async function POST(req: Request) {
  ...
  const rl = rateLimit(`matriculas:POST:${ip}`, 20, 60_000)
  ...
  const { data, error } = await supabase.from("matriculas").insert(toInsert).select("*").single()
