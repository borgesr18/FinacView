...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabase.from("atendimentos").select("*").order("data", { ascending: false }).limit(100)
...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabase.from("atendimentos").insert(toInsert).select("*").single()
